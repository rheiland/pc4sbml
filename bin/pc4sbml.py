...
def write_config_file_cb(b):
    # path_to_share = os.path.join('~', '.local','share','eigen_motility_app')
    path_to_share = os.path.join('~', '.local','share','pc4sbml')
...
def get_config_files():
    ...
    if nanoHUB_flag:
        full_path = os.path.expanduser("~/data/results/.submit_cache/pc4sbml")  # does Windows like this?
    else:
        # local cache
        try:
            cachedir = os.environ['CACHEDIR']
            full_path = os.path.join(cachedir, "pc4sbml")
        except:
...
# Executed after 'Run' IF you have hublib module (which is true on nanoHUB)
def run_done_func(s, rdir):
    ...
    if nanoHUB_flag:
        ...
        os.system("submit  mail2self -s 'nanoHUB pc4sbml' -t 'Your Run completed.'&")
...
#     # make sure we are where we started
    ...
    # os.system('rm -rf tmpdir*')   # rwh: temporarily pause this
...
#    svg.update(tdir)
    ...
    if nanoHUB_flag:
        if remote_cb.value:
            s.run(run_name, "-v ncn-hub_M@brown -n 8 -w 1440 pc4sbml-r7 config.xml")   # "-r7" suffix??
        else:
        ...
        # reset Dropdown 'Load Config' to 'DEFAULT' when Run interactively - rwh: HOW/WHY???
...
tool_title = widgets.Label(r'\(\textbf{PhysiCell with SBML}\)')
if nanoHUB_flag or hublib_flag:
