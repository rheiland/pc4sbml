...
#        scipy.io.loadmat(fullname, info_dict)
        ...
        if (self.cmap_fixed.value):
            try:
                my_plot = plt.contourf(xgrid, ygrid, M[self.field_index, :].reshape(self.numy, self.numx), levels=levels, extend='both', cmap=self.field_cmap.value, fontsize=self.fontsize)
            except:
                contour_ok = False
        ...
        oxy_ax = self.fig.add_subplot(grid[2, 0])
        x = np.linspace(0, 500)
...
# widgets.HBox([mcds_play, mcds_slider])
    ...
    def plot_oxygen(self, frame, grid):
        # self.fig = plt.figure(figsize=(18.0,15))  # this strange figsize results in a ~square contour plot
        self.fig = plt.figure(figsize=(15.0,15))  # this strange figsize results in a ~square contour plot
        plt.subplot(grid[2, 0])
        x = np.linspace(0, 10)
        plt.plot(np.sin(x))
        # ax1.plot(np.sin(x))
    ...
    def plot_plots(self, frame):
        # global current_idx, axes_max, gFileId, field_index
        grid = plt.GridSpec(3, 3, wspace=0.9, hspace=0.9)
